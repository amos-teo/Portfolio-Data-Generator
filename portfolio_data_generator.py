...
#Set index tickers needed
index_list = ['ICSH','^GSPC'] #money market and S&P 500
index_pct = [0.2,0.8] #20% money market, 80% S&P 500 Index


###If we allow user to set indexes, please use the lines of code below.
#index = 'index holder'
#while index != '':
#    index = input('Add index ticker over here -->  ')
#    if index == '':
#        break
#    pct = input('add percentage for each ticker respectively. i.e. 50% should be 0.40 --> ')
#    index_list = index_list + [index]
#    index_pct = index_pct + [pct]
...
#Place index list into security list to import security prices from yfinance
security_list = security_list + index_list
...
'''Step 8: Prepare Index with returns in separate columns, append back to daily_performance dataframe'''
#Create name of index
index_name = ''
for i in range(len(index_list)):
    if i == 0:
        index_name  = index_name + str(index_pct[i]) + index_list[i]
    else:
        index_name  = index_name + "+" + str(index_pct[i]) + index_list[i]

#create new dataframe to house index data before export
index_performance = pd.DataFrame({'date':pd.date_range(start = min_date + pd.DateOffset(days = -5) ,end = max_date + pd.DateOffset(days = -1) ) })
index_performance['date'] = index_performance.date.astype('datetime64[ns]')

#get price list from
index_tracker = 0 #index tracker to track whether to create new table or join to table
index_name = '' #use to assign index name
#loop through each index from user input to pull out prices
for index in index_list:
    index_import = security_master[security_master['Ticker'] == index][['Date','Adj Close']] #pull out table from security master
    index_import['index_'+str(index_tracker)+'_%'] = index_pct[index_tracker]

    #Create lag column, and use to calculate daily returns of each index
    index_import['lag_price'] = index_import['Adj Close'].shift(1)
    #Calculate daily returns by dividing today's price by yesterday's price
    index_import['Returns_'+str(index_tracker)] = index_import.apply(lambda x: x['Adj Close']/x['lag_price'] ,axis = 1)

    #merge dataframe to every date
    index_performance = pd.merge(index_performance,index_import,how = 'left',left_on = 'date',right_on= 'Date' )

    #Drop irrelevant columns
    index_performance = index_performance.drop(columns = ['Date','lag_price','Adj Close'])

    #Assign index name
    index_name = index_name + str(float(index_pct[index_tracker])*100) + '% ' + index + ' + '

    index_tracker+=1

#Remove irrelevant dates from index date range
index_performance = index_performance[index_performance['date'] >= min_date]

#Create new columns so that output will always have multiple index columns (to ensure Tableau workbook doesnt break)
if 'index_0_%' in index_performance:
    pass
else:
    index_performance['index_0_%'] = 0
    index_performance['Returns_0'] = 1

if 'index_1_%' in index_performance:
    pass
else:
    index_performance['index_1_%'] = 0
    index_performance['Returns_1'] = 1

#Assign index proportion + index name as account name, calculation type as Index
index_performance['calculation_type'] = 'Index' #Index filter
index_performance['account_name'] =  index_name[:-2] #Index Name, remove the final ' + ' from the string

#Concat index performance back to daily_performance dataframe
daily_performance = pd.concat([daily_performance,index_performance],axis = 0)

'''Step 9: Calculate asset breakdown to break down funds'''
